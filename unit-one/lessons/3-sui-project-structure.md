...
This creates: 
- the project root folder `HelloWorld`
...
This is the `Move.toml` generated by the Sui CLI with the package name `HelloWorld`:
...
- A Sui module name uses the Rust path separator `::` to divide the package name and the module name, examples:
    1. `helloworld::hello` - `hello` module in package referenced by alias `helloworld`
    2. `capy::capy` - `capy` module in package referenced by alias `capy`
